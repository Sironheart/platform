...
class OrderConverter
{
    ...
    public function assembleSalesChannelContext(OrderEntity $order, Context $context): SalesChannelContext
    {
        ...
        if (Feature::isActive('FEATURE_NEXT_6059')) {
            $item = $order->getItemRounding() ?? new CashRoundingConfig($salesChannelContext->getCurrency()->getDecimalPrecision(), 0.01, true);
            $total = $order->getTotalRounding() ?? new CashRoundingConfig($salesChannelContext->getCurrency()->getDecimalPrecision(), 0.01, true);

            $salesChannelContext->setItemRounding($item);
            $salesChannelContext->setTotalRounding($total);
        } else {
